...
const BearMenus = ({ lightThem, menuMode, siderOpenKeys, topHeaders, onMenuOpenChange, onSwitchTheme, onSwitchMenuMode, location }) => {
  ...
  return (
    <Menu {...menuProps} selectedKeys={Array.of(menukey)}>
      <SubMenu key="home" title={<span><Icon type="home" /><span>XXXX</span></span>}>
        <Menu.Item key="for"><Link to="/for">XXXX </Link></Menu.Item>
      ...
      {/*</SubMenu>*/}
      <SubMenu key="home" title={<span><Icon type="home" /><span>语音合成</span></span>}>
        <Menu.Item key="tts"><Link to="/tts">TTS </Link></Menu.Item>

      </SubMenu>
      <SubMenu key="home" title={<span><Icon type="home" /><span>Example</span></span>}>
        <Menu.Item key="example"><Link to="/example">Example1</Link></Menu.Item>
        <Menu.Item key="example"><Link to="/example">Example2</Link></Menu.Item>
      </SubMenu>
      <SubMenu key="home" title={<span><Icon type="home" /><span>翻译模块</span></span>}>
        <Menu.Item key="translate"><Link to="/translate">翻译</Link></Menu.Item>
        <Menu.Item key="translate"><Link to="/translate">翻译1</Link></Menu.Item>
      </SubMenu>
      <SubMenu key="home" title={<span><Icon type="home" /><span>听力视频</span></span>}>
        <Menu.Item key="audio"><Link to="/audio">听力</Link></Menu.Item>
        <Menu.Item key="AddAudio"><Link to="/AddAudio">添加听力</Link></Menu.Item>
        <Menu.Item key="video"><Link to="/video">视频</Link></Menu.Item>
        <Menu.Item key="AddVideo"><Link to="/AddVideo">添加视频</Link></Menu.Item>
      </SubMenu>
      <SubMenu key="home" title={<span><Icon type="home" /><span>从句</span></span>}>
        <Menu.Item key="Clause"><Link to="/Clause">英语语法</Link></Menu.Item>
        <Menu.Item key="translate"><Link to="/video"></Link></Menu.Item>
      </SubMenu>
      <SubMenu key="home" title={<span><Icon type="home" /><span>小测试</span></span>}>
      <Menu.Item key="mail">
        <Link to="/test"><Icon type="test" /><span>test</span></Link>
      </Menu.Item>
        <Menu.Item key="addTest">
        <Link to="/AddTest"><Icon type="test" /><span>添加试题</span></Link>
      </Menu.Item>
</SubMenu>
